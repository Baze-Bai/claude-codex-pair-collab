...
分两条的原因:`codex exec resume` 无 `-s`/`-C` 参数,**沙箱随会话持久化(2026-07-20 实测:workspace-write 建会话→裸 resume 仍 workspace-write [workdir,/tmp,$TMPDIR])**,读写分离最干净(切沙箱的逃生门 `-c sandbox_mode="workspace-write"` 仍未实测,慎用)。**与沙箱相反,model 与 reasoning effort 不随会话持久:每次调用(含 resume)都从 config.toml+CLI flags 重解析**(同日实测:建会话 effort=low,裸 resume 回落 config 默认 xhigh;resume 有一等 `-m`,也吃 `-c model_reasoning_effort=`)——据此 `cb-round.sh` 每次调用(含 resume)都显式下发 `-c model_reasoning_effort=`(默认 **xhigh**,即 skill 为 CB 设的 effort 默认档,env `CB_EFFORT` 可覆盖),把 effort 钉住不随 config 漂移。**model 不钉**:主题若为 CB 指定了模型,**每轮 resume 仍须显式 `-m` 重申**,否则用户中途改 App/config 默认会让进行中主题悄悄换模型。`-s workspace-write` 主路径:flag 接受、策略生效、随 resume 持久均已实测;**尚未实测的只剩模型真实写盘,「创建即冒烟」保留**(见 Phase 4)。

**默认经 `scripts/cb-round.sh` 调用**(`new`/`resume` 两模式:统一 stdin 姿势与日志、抓取并登记 session id、首次 `new` 顺带把 `codex --version` 记进 `codex_sessions.txt`、`resume` 以 per-UUID 锁强制单写者、产出过形式预检、每次调用显式钉 effort——默认 `xhigh`,env `CB_EFFORT` 覆盖)。下方原始命令是脚本封装的底层事实,也是脚本故障时的降级路径。

**创建会话(每条会话首次调用),捕获 session id:**

```bash
# Bash 工具(必须;PowerShell 后台跑会因 stdin 未关永久卡死)
codex exec -C "$(git rev-parse --show-toplevel)" -s read-only -c model_reasoning_effort=xhigh \
...
```bash
codex exec resume <SESSION_ID> -c model_reasoning_effort=xhigh -o "<输出文件>" - < "<prompt文件>"
```
